...
# Generate a genesis transaction that creates a validator with a self-delegation, that is signed by th
extern 'cyber gentx' [
	key_name: string
	amount: string
...
# A state rollback is performed to recover from an incorrect application state transition, when Tender
...
# testnet will create "v" number of directories and populate each with necessary files (private valida
...
# Add a genesis account to genesis.json. The provided account must specify the account address or key 
...
# Run the full node application with Tendermint in or out of process. By default, the application will
extern 'cyber start' [
...
# Delete keys from the Keybase backend. Note that removing offline or ledger keys will remove only the
...
# Migrate key information from the legacy (db-based) Keybase to the new keyring-based Keyring. The leg
...
# Convert and print to stdout key addresses and fingerprints from hexadecimal into bech32 cosmos prefi
...
# Broadcast transactions created with the --generate-only flag and signed with the sign command. Read 
...
# Encode transactions created with the --generate-only flag and signed with the sign command. Read a t
...
# Assemble a batch of multisig transactions generated by batch sign command. Read one or more signatur
...
# Sign a transaction created with the --generate-only flag. It will read a transaction from [file], si
...
# Print the addresses that must sign the transaction, those who have already signed it, and make sure 
...
extern 'cyber tx authz grant' [
	...
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
	--expiration: int		# The Unix timestamp. Default is one year. (default 1695367476)
	--fee-account: string		# Fee account pays fees for the transaction instead of deducting from the signer
	--fees: string		# Fees to pay along with transaction; eg: 10uatom
	--from: string		# Name or address of private key with which to sign
	--gas: string		# gas limit to set per-transaction; set to "auto" to calculate sufficient gas automatically (default 200000)
...
# Grant authorization to pay fees from your address. Note, the'--from' flag is 				ignored as it is im
...
# Deposit coins a liquidity pool. This deposit request is not processed immediately since it is accumu
...
# Withdraw pool coin from the specified liquidity pool. This swap request is not processed immediately
...
# create a new IBC client with the specified client state and consensus state 	- ClientState JSON exam
...
# Export a private key from the local keyring in ASCII-armored encrypted format. When both the --unarm
...
# Create a bip39 mnemonic, sometimes called a seed phrase, by reading from the system entropy. To pass
...
# revoke fee grant from a granter to a grantee. Note, the'--from' flag is 			ignored as it is implied 
...
# Search for transactions that match the exact given events where results are paginated. Each event ta
...
# Withdraw all rewards for a single delegator. Note that if you use this command with --broadcast-mode
...
# Submit a deposit for an active proposal. You can find the proposal-id by running "cyber query gov pr
...
# Submit a vote for an active proposal. You can find the proposal-id by running "cyber query gov propo
...
# Transfer a fungible token through IBC. Timeouts can be specified as absolute or relative using the "
...
# Query details for all deposits on a proposal. You can find the proposal-id by running "cyber query g
...
# Query tally of votes on a proposal. You can find the proposal-id by running "cyber query gov proposa
...
# Use a validators' consensus public key to find the signing-info for that validator: $ <appd> query s
...
# Help provides help for any command in the application. Simply type cyber help [path to command] for 
extern 'cyber help' [
	command: string
...
# Derive a new private key and encrypt to disk. Optionally specify a BIP39 mnemonic, a BIP39 passphras
...
# Display keys details. If multiple names or addresses are provided, then an ephemeral multisig key wi
...
# Sign transactions created with the --generate-only flag that require multisig signatures. Read one o
...
# upgrade the IBC client associated with the provided client identifier while providing proof committe
...
# Send funds from one account to another. Note, the'--from' flag is ignored as it is implied from [fro
...
# Withdraw rewards from a given delegation address, and optionally withdraw validator commission if th
...
# Query authorization grants for a granter-grantee pair. If msg-type-url is set, it will select grants
...
# Query the deposit messages on the liquidity pool batch for the specified pool-id and msg-index If ba
...
# Query all swap messages in the liquidity pool batch for the specified pool-id If batch messages are 
...
# Query all withdraw messages on the liquidity pool batch for the specified pool-id If batch messages 
...
# Sign batch files of transactions generated with --generate-only. The command processes list of trans
...
# Swap offer coin with demand coin from the liquidity pool with the given order price. This swap reque
...
# Create a new vesting account funded with an allocation of tokens. The account can either be a delaye
...
# Query the withdraw messages in the liquidity pool batch for the specified pool-id and msg-index if t
...
# If upgrade-name was previously executed on the chain, this returns the header for the block at which
...
# Submit a parameter proposal along with an initial deposit. The proposal details must be supplied via
...
# Submit an update IBC client proposal along with an initial deposit. Please specify a subject client 
...
# Return a list of all public keys stored by this key manager along with their associated name and add
...
# Query all deposit messages of the liquidity pool batch on the specified pool If batch messages are n
...
# Gets a list of module names and their respective consensus versions. Following the command with a sp
...
# Submit a community pool spend proposal along with an initial deposit. The proposal details must be s
...
# Submit a software upgrade along with an initial deposit. Please specify a unique name and height for
...
# Submit an update instantiate config  proposal for multiple code ids. Example:  $ cyber tx gov submit
...
# Query for the swap message on the batch of the liquidity pool specified pool-id and msg-index If the
...
# Submit an IBC client breaking upgrade proposal along with an initial deposit. The client state speci
...
# Given a list of acknowledgement sequences from counterparty, determine if an ack on the counterparty
...
# Determine if a packet, given a list of packet commitment sequences, is unreceived. The return value 
...
# Query the consensus state for a particular light client at a given height. If the '--latest' flag is
...
# Query the self consensus state for this chain. This result may be used for verifying IBC clients rep
