...
extern 'gaiad query tendermint-validator-set' [
	height: string
...
extern 'gaiad tendermint reset-state' [
...
# Show this node's ID
extern 'gaiad tendermint show-node-id' [
...
extern 'gaiad tendermint unsafe-reset-all' [
...
extern 'gaiad tx multisign-batch' [
	file: string
	name: string
...
extern 'gaiad query authz grants-by-grantee' [
	grantee_addr: string
...
extern 'gaiad query gov deposit' [
	proposal_id: string
...
extern 'gaiad query gov proposal' [
	proposal_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query gov proposer' [
	proposal_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc-router params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query mint inflation' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query slashing params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query slashing signing-infos' [
...
extern 'gaiad tx authz grant' [
	...
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
	--expiration: int		# The Unix timestamp. Default is one year. (default 1695572837)
	--fee-account: string		# Fee account pays fees for the transaction instead of deducting from the signer
	--fees: string		# Fees to pay along with transaction; eg: 10uatom
	--from: string		# Name or address of private key with which to sign
	--gas: string		# gas limit to set per-transaction; set to "auto" to calculate sufficient gas automatically (default 200000)
...
extern 'gaiad tx feegrant grant' [
...
extern 'gaiad tx liquidity create-pool' [
	pool_type: string
...
extern 'gaiad tx liquidity swap' [
	pool_id: string
	swap_type: string
	offer_coin: string
...
extern 'gaiad tx staking create-validator' [
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--amount: string		# Amount of coins to bond
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--commission-max-change-rate: string		# The maximum commission change rate percentage (per day)
...
extern 'gaiad tx staking edit-validator' [
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad tx staking unbond' [
...
extern 'gaiad query interchain-accounts host packet-events' [
	channel_id: string
	sequence: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad add-genesis-account' [
...
extern 'gaiad tendermint show-address' [
...
extern 'gaiad tendermint version' [
	--help(-h)		# help for version
	--home: string		# directory for config and data (default "/Users/user/.gaia")
	--log_format: string		# The logging format (json|plain) (default "plain")
	--log_level: string		# The logging level (trace|debug|info|warn|error|fatal|panic) (default "info")
...
extern 'gaiad tx validate-signatures' [
	file: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
...
extern 'gaiad query authz grants-by-granter' [
	granter_addr: string
...
extern 'gaiad query mint params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad tx feegrant revoke' [
	granter: string
	grantee: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad tx liquidity deposit' [
	pool_id: string
	deposit_coins: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad tx staking delegate' [
	validator_addr: string
	amount: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad tx vesting create-vesting-account' [
	to_address: string
	amount: string
	end_time: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
...
extern 'gaiad query bank denom-metadata' [
...
extern 'gaiad query feegrant grant' [
	granter: string
	grantee: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for grant
...
extern 'gaiad query feegrant grants-by-granter' [
	granter: string
...
extern 'gaiad query ibc-transfer denom-trace' [
	hash: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc-transfer escrow-address' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query params subspace' [
	subspace: string
	key: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx ibc-transfer transfer' [
	src_port: string
	src_channel: string
	receiver: string
	amount: string
...
extern 'gaiad query gov deposits' [
	proposal_id: string
...
extern 'gaiad query slashing signing-info' [
...
extern 'gaiad tx staking redelegate' [
...
extern 'gaiad query bank balances' [
	address: string
...
extern 'gaiad query feegrant grants-by-grantee' [
	grantee: string
	--count-total		# count total number of records in grants to query for
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for grants-by-grantee
...
extern 'gaiad query ibc-transfer denom-traces' [
...
extern 'gaiad tx crisis invariant-broken' [
	module_name: string
...
extern 'gaiad query distribution commission' [
	validator: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query distribution params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query distribution slashes' [
	validator: string
	start_height: string
	end_height: string
...
extern 'gaiad query liquidity batch' [
	pool_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query liquidity deposits' [
	pool_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for deposits
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
...
extern 'gaiad query liquidity pool' [
	pool_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query liquidity swap' [
	pool_id: string
	msg_index: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for swap
...
extern 'gaiad query liquidity withdraw' [
	pool_id: string
	msg_index: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query upgrade applied' [
	upgrade_name: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query upgrade plan' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx slashing unjail' [
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
	--fee-account: string		# Fee account pays fees for the transaction instead of deducting from the signer
...
extern 'gaiad tx ibc client misbehaviour' [
...
extern 'gaiad tx ibc client upgrade' [
...
extern 'gaiad query authz grants' [
	granter_addr: string
	grantee_addr: string
	msg_type_url: string
	--count-total		# count total number of records in grants to query for
...
extern 'gaiad query mint annual-provisions' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx liquidity withdraw' [
	pool_id: string
	pool_coin: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad query ibc-transfer params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query distribution community-pool' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query distribution validator-outstanding-rewards' [
	validator: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query liquidity params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query liquidity swaps' [
	pool_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query upgrade module_versions' [
...
extern 'gaiad tx ibc client create' [
	path_to_client_state_json: string
	path_to_consensus_state_json: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
# Query for account by address
extern 'gaiad query auth account' [
	address: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for account
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
...
extern 'gaiad query auth params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query staking delegations' [
...
extern 'gaiad query staking historical-info' [
	height: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query staking pool' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for pool
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query staking redelegations' [
	delegator_addr: string
...
extern 'gaiad query staking unbonding-delegation' [
	delegator_addr: string
	validator_addr: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query staking unbonding-delegations-from' [
	validator_addr: string
...
extern 'gaiad query staking validators' [
...
extern 'gaiad tx distribution set-withdraw-addr' [
...
extern 'gaiad tx distribution withdraw-rewards' [
	validator_addr: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
...
extern 'gaiad tx gov submit-proposal community-pool-spend' [
...
extern 'gaiad tx gov submit-proposal param-change' [
	proposal_file: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
...
extern 'gaiad tx gov submit-proposal update-client' [
...
extern 'gaiad tendermint show-validator' [
...
extern 'gaiad query gov proposals' [
...
extern 'gaiad query interchain-accounts host params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query ibc-transfer denom-hash' [
	trace: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query distribution rewards' [
	delegator_addr: string
	validator_addr: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query liquidity pools' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx ibc client update' [
	client_id: string
...
extern 'gaiad query staking delegation' [
	delegator_addr: string
	validator_addr: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query staking params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query staking redelegations-from' [
	validator_addr: string
...
extern 'gaiad query staking validator' [
	validator_addr: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx distribution withdraw-all-rewards' [
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
	--fee-account: string		# Fee account pays fees for the transaction instead of deducting from the signer
...
extern 'gaiad tx gov submit-proposal ibc-upgrade' [
	name: string
	height: string
...
extern 'gaiad query interchain-accounts controller params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query liquidity withdraws' [
	pool_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query auth accounts' [
...
extern 'gaiad query staking redelegation' [
	delegator_addr: string
	src_validator_addr: string
	dst_validator_addr: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad tx distribution fund-community-pool' [
	amount: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--dry-run		# ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
...
extern 'gaiad tx gov submit-proposal software-upgrade' [
	name: string
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--deposit: string		# deposit of proposal
...
extern 'gaiad query ibc client consensus-state' [
	client_id: string
	height: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc client header' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc client self-consensus-state' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc client states' [
...
extern 'gaiad query liquidity deposit' [
	pool_id: string
	msg_index: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for deposit
...
extern 'gaiad query staking delegations-to' [
	validator_addr: string
...
extern 'gaiad tx gov submit-proposal cancel-software-upgrade' [
	--account-number(-a): int		# The account number of the signing account (offline mode only)
	--broadcast-mode(-b): string		# Transaction broadcasting mode (sync|async|block) (default "sync")
	--deposit: string		# deposit of proposal
	--description: string		# description of proposal
...
extern 'gaiad query ibc client params' [
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for params
	--node: string		# <host>:<port> to Tendermint RPC interface for this chain (default "tcp://localhost:26657")
	--output(-o): string		# Output format (text|json) (default "text")
...
extern 'gaiad query ibc client status' [
	client_id: string
	--help(-h)		# help for status
	--chain-id: string		# The network chain ID
	--home: string		# directory for config and data (default "/Users/user/.gaia")
...
extern 'gaiad query ibc connection end' [
...
extern 'gaiad query staking unbonding-delegations' [
	delegator_addr: string
	--count-total		# count total number of records in unbonding delegations to query for
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc client consensus-states' [
	client_id: string
...
extern 'gaiad query ibc connection connections' [
...
# Submit a vote for an active proposal. You can find the proposal-id by running "gaiad query gov proposals".
extern 'gaiad tx gov weighted-vote' [
	proposal_id: string
...
extern 'gaiad query ibc client state' [
	client_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel channels' [
...
extern 'gaiad query ibc channel connections' [
	connection_id: string
...
extern 'gaiad query ibc channel next-sequence-receive' [
	port_id: string
	channel_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel packet-commitment' [
	port_id: string
	channel_id: string
	sequence: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel packet-receipt' [
	port_id: string
	channel_id: string
	sequence: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel unreceived-packets' [
	port_id: string
	channel_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc connection path' [
	client_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel end' [
	port_id: string
	channel_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
	--help(-h)		# help for end
...
extern 'gaiad query ibc channel packet-commitments' [
	port_id: string
	channel_id: string
...
extern 'gaiad query ibc channel client-state' [
	port_id: string
	channel_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel unreceived-acks' [
	port_id: string
	channel_id: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
...
extern 'gaiad query ibc channel packet-ack' [
	port_id: string
	channel_id: string
	sequence: string
	--height: int		# Use a specific height to query state at (this can error if the node is pruning state)
