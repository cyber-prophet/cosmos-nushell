...
Usage:
  cyber add-genesis-account [address_or_key_name] [coin][,[coin]] [flags]
...
Flags:
  ...
  -h, --help                                help for gentx
      ...
      --ip string                           The node's public IP (default "192.168.8.107")
...
Use "cyber keys [command] --help" for more information about a command.
...
Usage:
  cyber keys show [name_or_address [name_or_address...]] [flags]
...
Usage:
  cyber query account [address] [flags]
...
Usage:
  cyber query tendermint-validator-set [height] [flags]
...
Example:
$ cyber query tx <hash>
$ cyber query tx --type=acc_seq <addr>/<sequence>
$ cyber query tx --type=signature <sig1_base64>,<sig2_base64...>

Usage:
  cyber query tx --type=[hash|acc_seq|signature] [hash|acc_seq|signature] [flags]
...
Example:
$ cyber query txs --events 'message.sender=cosmos1...&message.action=withdraw_delegator_reward' --page 1 --limit 30

Usage:
  cyber query txs [flags]
...
### cyber tendermint unsafe-reset-all
...
Usage:
  cyber tx broadcast [file_path] [flags]
...
Usage:
  cyber tx decode [amino-byte-string] [flags]
...
Use "cyber tx gov [command] --help" for more information about a command.
...
Use "cyber tx ibc [command] --help" for more information about a command.

```

### cyber tx ibc-transfer
...
Example:
$ cyber tx multisign transaction.json k1k2k3 k1sig.json k2sig.json k3sig.json
...
Usage:
  cyber tx multisign [file] [name] [[signature]...] [flags]
...
Example:
$ cyber tx multisign-batch transactions.json multisigk1k2k3 k1sigs.json k2sigs.json k3sig.json

The current multisig implementation defaults to amino-json sign mode.
The SIGN_MODE_DIRECT sign mode is not supported.'

Usage:
  cyber tx multisign-batch [file] [name] [[signature-file]...] [flags]
...
Usage:
  cyber tx sign-batch [file] [flags]
...
Usage:
  cyber tx staking [flags]
  cyber tx staking [command]
...
Use "cyber tx staking [command] --help" for more information about a command.

```

### cyber tx validate-signatures
...
Usage:
  cyber tx validate-signatures [file] [flags]
...
Usage:
  cyber query auth account [address] [flags]
...
Usage:
  cyber query auth accounts [flags]
...
Examples:
$ cyber q authz grants-by-grantee cosmos1skj..

Usage:
  cyber query authz grants-by-grantee [grantee-addr] [flags]
...
Usage:
  cyber query bandwidth load [flags]
...
Usage:
  cyber query bandwidth neuron [address] [flags]
...
Usage:
  cyber query bandwidth params [flags]
...
Usage:
  cyber query bandwidth price [flags]
...
Usage:
  cyber query bandwidth total [flags]
...
Example:
  $ cyber query bank balances [address]
  $ cyber query bank balances [address] --denom=[denom]

Usage:
  cyber query bank balances [address] [flags]
...
To query for the client metadata of a specific coin denomination use:
  $ cyber query bank denom-metadata --denom=[denom]

Usage:
  cyber query bank denom-metadata [flags]
...
Example:
$ cyber query distribution commission bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj

Usage:
  cyber query distribution commission [validator] [flags]
...
Usage:
  cyber query distribution community-pool [flags]
...
Usage:
  cyber query distribution params [flags]
...
Usage:
  cyber query dmn thought [program] [name] [flags]
...
Usage:
  cyber query dmn thought-stats [program] [name] [flags]
...
Usage:
  cyber query dmn thoughts [flags]
...
Usage:
  cyber query dmn thoughts-stats [flags]
...
Example:
$ cyber query feegrant grants-by-granter [granter]

Usage:
  cyber query feegrant grants-by-granter [granter] [flags]
...
Example:
$ cyber query gov deposit 1 cosmos1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk

Usage:
  cyber query gov deposit [proposal-id] [depositer-addr] [flags]
...
### cyber query gov deposits

```
Query details for all deposits on a proposal.
You can find the proposal-id by running "cyber query gov proposals".

Example:
$ cyber query gov deposits 1

Usage:
  cyber query gov deposits [proposal-id] [flags]
...
### cyber query gov param

```
Query the all the parameters for the governance process.

Example:
$ cyber query gov param voting
$ cyber query gov param tallying
$ cyber query gov param deposit

Usage:
  cyber query gov param [param-type] [flags]
...
### cyber query gov proposal

```
Query details for a proposal. You can find the
proposal-id by running "cyber query gov proposals".

Example:
$ cyber query gov proposal 1

Usage:
  cyber query gov proposal [proposal-id] [flags]
...
### cyber query gov proposals

```
Query for a all paginated proposals that match optional filters:

Example:
$ cyber query gov proposals --depositor cosmos1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk
$ cyber query gov proposals --voter cosmos1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk
$ cyber query gov proposals --status (DepositPeriod|VotingPeriod|Passed|Rejected)
$ cyber query gov proposals --page=2 --limit=100

Usage:
  cyber query gov proposals [flags]
...
### cyber query gov proposer

```
Query which address proposed a proposal with a given ID.

Example:
$ cyber query gov proposer 1

Usage:
  cyber query gov proposer [proposal-id] [flags]
...
### cyber query gov tally

```
Query tally of votes on a proposal. You can find
the proposal-id by running "cyber query gov proposals".

Example:
$ cyber query gov tally 1

Usage:
  cyber query gov tally [proposal-id] [flags]
...
### cyber query gov vote

```
Query details for a single vote on a proposal given its identifier.

Example:
$ cyber query gov vote 1 cosmos1skjwj5whet0lpe65qaq4rpq03hjxlwd9nf39lk

Usage:
  cyber query gov vote [proposal-id] [voter-addr] [flags]
...
### cyber query gov votes

```
Query vote details for a single proposal by its identifier.

Example:
$ cyber query gov votes 1
$ cyber query gov votes 1 --page=2 --limit=100

Usage:
  cyber query gov votes [proposal-id] [flags]
...
### cyber query graph stats

```
Query the graph stats

Usage:
  cyber query graph stats [flags]
...
### cyber query ibc-transfer escrow-address

```
Get the escrow address for a channel

Usage:
  cyber query ibc-transfer escrow-address [flags]

Examples:
cyber query ibc-transfer escrow-address [port] [channel-id]
...
### cyber query ibc-transfer params

```
Query the current ibc-transfer parameters

Usage:
  cyber query ibc-transfer params [flags]
...
### cyber query liquidity deposits
...
Usage:
  cyber query liquidity params [flags]
...
### cyber query liquidity withdraw
...
### cyber query liquidity withdraws
...
### cyber query mint annual-provisions

```
Query the current minting annual provisions value

Usage:
  cyber query mint annual-provisions [flags]
...
### cyber query mint inflation

```
Query the current minting inflation value

Usage:
  cyber query mint inflation [flags]
...
### cyber query mint params

```
Query the current minting parameters

Usage:
  cyber query mint params [flags]
...
### cyber query params subspace

```
Query for raw parameters by subspace and key

Usage:
  cyber query params subspace [subspace] [key] [flags]
...
### cyber query rank karma

```
Query the current karma of given neuron

Usage:
  cyber query rank karma [neuron] [flags]
...
### cyber query rank rank

```
Query the current rank of given particle

Usage:
  cyber query rank rank [particle] [flags]
...
### cyber query rank top

```
Query top

Usage:
  cyber query rank top [flags]
...
### cyber query resources investmint
...
Usage:
  cyber query slashing params [flags]
...
### cyber query slashing signing-info
...
Usage:
  cyber query slashing signing-info [validator-conspub] [flags]
...
### cyber query slashing signing-infos
...
Usage:
  cyber query slashing signing-infos [flags]
...
### cyber query staking delegation
...
### cyber query staking delegations

```
Query delegations for an individual delegator on all validators.

Example:
$ cyber query staking delegations bostrom1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p

Usage:
  cyber query staking delegations [delegator-addr] [flags]
...
### cyber query staking delegations-to

```
Query delegations on an individual validator.

Example:
$ cyber query staking delegations-to bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj

Usage:
  cyber query staking delegations-to [validator-addr] [flags]
...
### cyber query staking historical-info
...
Usage:
  cyber query staking params [flags]
...
Usage:
  cyber query staking pool [flags]
...
### cyber query staking redelegation
...
### cyber query staking redelegations

```
Query all redelegation records for an individual delegator.

Example:
$ cyber query staking redelegation bostrom1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p

Usage:
  cyber query staking redelegations [delegator-addr] [flags]
...
### cyber query staking redelegations-from

```
Query delegations that are redelegating _from_ a validator.

Example:
$ cyber query staking redelegations-from bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj

Usage:
  cyber query staking redelegations-from [validator-addr] [flags]
...
### cyber query staking unbonding-delegation
...
### cyber query staking unbonding-delegations

```
Query unbonding delegations for an individual delegator.

Example:
$ cyber query staking unbonding-delegations bostrom1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p

Usage:
  cyber query staking unbonding-delegations [delegator-addr] [flags]
...
### cyber query staking unbonding-delegations-from

```
Query delegations that are unbonding _from_ a validator.

Example:
$ cyber query staking unbonding-delegations-from bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj

Usage:
  cyber query staking unbonding-delegations-from [validator-addr] [flags]
...
### cyber query staking validator

```
Query details about an individual validator.

Example:
$ cyber query staking validator bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj

Usage:
  cyber query staking validator [validator-addr] [flags]
...
Usage:
  cyber query upgrade applied [upgrade-name] [flags]
...
Usage:
  cyber query upgrade plan [flags]
...
### cyber query wasm code

```
Downloads wasm bytecode for given code id

Usage:
  cyber query wasm code [code_id] [output filename] [flags]
...
### cyber query wasm code-info

```
Prints out metadata of a code id

Usage:
  cyber query wasm code-info [code_id] [flags]
...
### cyber query wasm contract-history

```
Prints out the code history for a contract given its address

Usage:
  cyber query wasm contract-history [bech32_address] [flags]
...
### cyber query wasm contract-state
...
### cyber query wasm libwasmvm-version
...
### cyber query wasm list-code

```
List all wasm bytecode on the chain

Usage:
  cyber query wasm list-code [flags]
...
### cyber query wasm list-contract-by-code
...
Example:
 $ cyber tx authz exec tx.json --from grantee
 $ cyber tx bank send <granter> <recipient> --from <granter> --chain-id <chain-id> --generate-only > tx.json && cyber tx authz exec tx.json --from grantee

Usage:
  cyber tx authz exec [msg_tx_json_file] --from [grantee] [flags]
...
Flags:
  ...
  -b, --broadcast-mode string        Transaction broadcasting mode (sync|async|block) (default "sync")
      ...
      --dry-run                      ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
      --expiration int               The Unix timestamp. Default is one year. (default 1694345962)
      --fee-account string           Fee account pays fees for the transaction instead of deducting from the signer
      --fees string                  Fees to pay along with transaction; eg: 10uatom
      --from string                  Name or address of private key with which to sign
      --gas string                   gas limit to set per-transaction; set to "auto" to calculate sufficient gas automatically (default 200000)
...
### cyber tx authz revoke

```
revoke authorization from a granter to a grantee:
Example:
 $ cyber tx authz revoke cosmos1skj.. /cosmos.bank.v1beta1.MsgSend --from=cosmos1skj..

Usage:
  cyber tx authz revoke [grantee] [msg_type] --from=[granter] [flags]
...
### cyber tx crisis invariant-broken

```
Submit proof that an invariant broken to halt the chain

Usage:
  cyber tx crisis invariant-broken [module-name] [invariant-route] [flags]
...
### cyber tx distribution fund-community-pool

```
Funds the community pool with the specified amount

Example:
$ cyber tx distribution fund-community-pool 100uatom --from mykey

Usage:
  cyber tx distribution fund-community-pool [amount] [flags]
...
### cyber tx distribution set-withdraw-addr

```
Set the withdraw address for rewards associated with a delegator address.

Example:
$ cyber tx distribution set-withdraw-addr bostrom1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p --from mykey

Usage:
  cyber tx distribution set-withdraw-addr [withdraw-addr] [flags]
...
Example:
$ cyber tx distribution withdraw-all-rewards --from mykey

Usage:
  cyber tx distribution withdraw-all-rewards [flags]
...
### cyber tx distribution withdraw-rewards
...
Example:
$ cyber tx distribution withdraw-rewards bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj --from mykey
$ cyber tx distribution withdraw-rewards bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj --from mykey --commission

Usage:
  cyber tx distribution withdraw-rewards [validator-addr] [flags]
...
Examples:
cyber tx feegrant grant cosmos1skjw... cosmos1skjw... --spend-limit 100stake --expiration 2022-01-30T15:04:05Z or
cyber tx feegrant grant cosmos1skjw... cosmos1skjw... --spend-limit 100stake --period 3600 --period-limit 10stake --expiration 36000 or
cyber tx feegrant grant cosmos1skjw... cosmos1skjw... --spend-limit 100stake --expiration 2022-01-30T15:04:05Z 
	--allowed-messages "/cosmos.gov.v1beta1.MsgSubmitProposal,/cosmos.gov.v1beta1.MsgVote"

Usage:
  cyber tx feegrant grant [granter_key_or_address] [grantee] [flags]
...
Example:
 $ cyber tx feegrant revoke cosmos1skj.. cosmos1skj..

Usage:
  cyber tx feegrant revoke [granter] [grantee] [flags]
...
### cyber tx gov deposit

```
Submit a deposit for an active proposal. You can
find the proposal-id by running "cyber query gov proposals".

Example:
$ cyber tx gov deposit 1 10stake --from mykey

Usage:
  cyber tx gov deposit [proposal-id] [deposit] [flags]
...
Example:
$ cyber tx gov vote 1 yes --from mykey

Usage:
  cyber tx gov vote [proposal-id] [option] [flags]
...
Example:
$ cyber tx gov weighted-vote 1 yes=0.6,no=0.3,abstain=0.05,no_with_veto=0.05 --from mykey

Usage:
  cyber tx gov weighted-vote [proposal-id] [weighted-options] [flags]
...
### cyber tx graph cyberlink

```
Create cyberlink.
Example:
$ cyber tx link cyberlink QmWZYRj344JSLShtBnrMS4vw5DQ2zsGqrytYKMqcQgEneB QmfZwbahFLTcB3MTMT8TA8si5khhRmzm7zbHToo4WVK3zn
...
### cyber tx grid delete-route

```
Delete your grid route to given destination address

Usage:
  cyber tx grid delete-route [destination] [flags]
...
### cyber tx grid edit-route

```
Set value of grid route to destination address

Usage:
  cyber tx grid edit-route [destination] [value] [flags]
...
### cyber tx grid edit-route-name

```
Edit name of grid route to given destination address

Usage:
  cyber tx grid edit-route-name [destination] [name] [flags]
...
### cyber tx ibc channel

```
IBC channel transaction subcommands

Usage:
  cyber tx ibc channel [flags]
...
### cyber tx liquidity create-pool

```
Create liquidity pool and deposit coins.

Example:
$ cyber tx liquidity create-pool 1 1000000000uatom,50000000000uusd --from mykey
...
Usage:
  cyber tx liquidity create-pool [pool-type] [deposit-coins] [flags]
...
Example:
$ cyber tx liquidity deposit 1 100000000uatom,5000000000uusd --from mykey
...
Usage:
  cyber tx liquidity deposit [pool-id] [deposit-coins] [flags]
...
Usage:
  cyber tx staking create-validator [flags]
...
### cyber tx staking delegate

```
Delegate an amount of liquid coins to a validator from your wallet.

Example:
$ cyber tx staking delegate bostromvaloper1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 1000stake --from mykey

Usage:
  cyber tx staking delegate [validator-addr] [amount] [flags]
...
### cyber tx staking redelegate

```
Redelegate an amount of illiquid staking tokens from one validator to another.

Example:
$ cyber tx staking redelegate bostromvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj bostromvaloper1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 100stake --from mykey

Usage:
  cyber tx staking redelegate [src-validator-addr] [dst-validator-addr] [amount] [flags]
...
### cyber tx vesting create-vesting-account
...
Usage:
  cyber tx vesting create-vesting-account [to_address] [amount] [end_time] [flags]
...
### cyber tx wasm clear-contract-admin

```
Clears admin for a contract to prevent further migrations

Usage:
  cyber tx wasm clear-contract-admin [contract_addr_bech32] [flags]
...
Usage:
  cyber tx wasm execute [contract_addr_bech32] [json_encoded_send_args] --amount [coins,optional] [flags]
...
### cyber tx wasm migrate

```
Migrate a wasm contract to a new code version

Usage:
  cyber tx wasm migrate [contract_addr_bech32] [new_code_id_int64] [json_encoded_migration_args] [flags]
...
### cyber tx wasm set-contract-admin

```
Set new admin for a contract

Usage:
  cyber tx wasm set-contract-admin [contract_addr_bech32] [new_admin_addr_bech32] [flags]
...
Usage:
  cyber query ibc channel channels [flags]
...
Usage:
  cyber query ibc channel next-sequence-receive [port-id] [channel-id] [flags]

Examples:
cyber query ibc channel next-sequence-receive [port-id] [channel-id]
...
Usage:
  cyber query ibc channel packet-commitment [port-id] [channel-id] [sequence] [flags]

Examples:
cyber query ibc channel packet-commitment [port-id] [channel-id] [sequence]
...
Usage:
  cyber query ibc channel packet-receipt [port-id] [channel-id] [sequence] [flags]

Examples:
cyber query ibc channel packet-receipt [port-id] [channel-id] [sequence]
...
Usage:
  cyber query ibc channel unreceived-acks [port-id] [channel-id] [flags]

Examples:
cyber query ibc channel unreceived-acks [port-id] [channel-id] --sequences=1,2,3
...
Usage:
  cyber query ibc channel unreceived-packets [port-id] [channel-id] [flags]

Examples:
cyber query ibc channel unreceived-packets [port-id] [channel-id] --sequences=1,2,3
...
Usage:
  cyber query ibc client consensus-state [client-id] [height] [flags]

Examples:
cyber query ibc client  consensus-state [client-id] [height]
...
Usage:
  cyber query ibc client consensus-states [client-id] [flags]

Examples:
cyber query ibc client consensus-states [client-id]
...
Usage:
  cyber query ibc client params [flags]
...
Usage:
  cyber query ibc client self-consensus-state [flags]
...
Usage:
  cyber query ibc client state [client-id] [flags]

Examples:
cyber query ibc client state [client-id]
...
Usage:
  cyber query ibc client states [flags]
...
Usage:
  cyber query ibc connection connections [flags]
...
Usage:
  cyber query ibc connection end [connection-id] [flags]

Examples:
cyber query ibc connection end [connection-id]
...
### cyber tx gov submit-proposal cancel-software-upgrade

```
Cancel a software upgrade along with an initial deposit.

Usage:
  cyber tx gov submit-proposal cancel-software-upgrade [flags]
...
### cyber tx gov submit-proposal clear-contract-admin

```
Submit a clear admin for a contract to prevent further migrations proposal

Usage:
  cyber tx gov submit-proposal clear-contract-admin [contract_addr_bech32] [flags]
...
### cyber tx gov submit-proposal community-pool-spend
...
Example:
$ cyber tx gov submit-proposal community-pool-spend <path/to/proposal.json> --from=<key_or_address>
...
Usage:
  cyber tx gov submit-proposal community-pool-spend [proposal-file] [flags]
...
### cyber tx gov submit-proposal execute-contract

```
Submit a execute wasm contract proposal (run by any address)

Usage:
  cyber tx gov submit-proposal execute-contract [contract_addr_bech32] [json_encoded_migration_args] [flags]
...
### cyber tx gov submit-proposal ibc-upgrade
...
### cyber tx gov submit-proposal instantiate-contract

```
Submit an instantiate wasm contract proposal

Usage:
  cyber tx gov submit-proposal instantiate-contract [code_id_int64] [json_encoded_init_args] --label [text] --title [text] --description [text] --run-as [address] --admin [address,optional] --amount [coins,optional] [flags]
...
### cyber tx gov submit-proposal migrate-contract

```
Submit a migrate wasm contract to a new code version proposal

Usage:
  cyber tx gov submit-proposal migrate-contract [contract_addr_bech32] [new_code_id_int64] [json_encoded_migration_args] [flags]
...
### cyber tx gov submit-proposal param-change
...
Example:
$ cyber tx gov submit-proposal param-change <path/to/proposal.json> --from=<key_or_address>
...
Usage:
  cyber tx gov submit-proposal param-change [proposal-file] [flags]
...
### cyber tx gov submit-proposal pin-codes

```
Submit a pin code proposal for pinning a code to cache

Usage:
  cyber tx gov submit-proposal pin-codes [code-ids] [flags]
...
### cyber tx gov submit-proposal set-contract-admin

```
Submit a new admin for a contract proposal

Usage:
  cyber tx gov submit-proposal set-contract-admin [contract_addr_bech32] [new_admin_addr_bech32] [flags]
...
### cyber tx gov submit-proposal software-upgrade
...
Usage:
  cyber tx gov submit-proposal software-upgrade [name] (--upgrade-height [height]) (--upgrade-info [info]) [flags]
...
Usage:
  cyber tx gov submit-proposal sudo-contract [contract_addr_bech32] [json_encoded_migration_args] [flags]
...
### cyber tx gov submit-proposal unpin-codes

```
Submit a unpin code proposal for unpinning a code to cache

Usage:
  cyber tx gov submit-proposal unpin-codes [code-ids] [flags]
...
### cyber tx gov submit-proposal update-client
...
Usage:
  cyber tx gov submit-proposal update-client [subject-client-id] [substitute-client-id] [flags]
...
### cyber tx gov submit-proposal update-instantiate-config
...
### cyber tx gov submit-proposal wasm-store
...
Usage:
  cyber tx ibc client create [path/to/client_state.json] [path/to/consensus_state.json] [flags]

Examples:
cyber tx ibc client create [path/to/client_state.json] [path/to/consensus_state.json] --from node0 --home ../node0/<app>cli --chain-id $CID
...
### cyber tx ibc client misbehaviour
...
Usage:
  cyber tx ibc client upgrade [client-identifier] [path/to/client_state.json] [path/to/consensus_state.json] [upgrade-client-proof] [upgrade-consensus-state-proof] [flags]

Examples:
cyber tx ibc client upgrade [client-identifier] [path/to/client_state.json] [path/to/consensus_state.json] [client-state-proof] [consensus-state-proof] --from node0 --home ../node0/<app>cli --chain-id $CID
...
Flags:
  ...
  -b, --broadcast-mode string        Transaction broadcasting mode (sync|async|block) (default "sync")
      --deny-validators strings      Deny validators addresses separated by ,
      --dry-run                      ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
      --expiration int               The Unix timestamp. Default is one year. (default 1694346497)
      --fee-account string           Fee account pays fees for the transaction instead of deducting from the signer
      --fees string                  Fees to pay along with transaction; eg: 10uatom
      --from string                  Name or address of private key with which to sign
      --gas string                   gas limit to set per-transaction; set to "auto" to calculate sufficient gas automatically (default 200000)
